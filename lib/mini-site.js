...
function academyApplication(lab) {
  return `<section class="app-card academy-app">
    <div class="app-toolbar"><div><small>SIMULADOR DECLARATIVO</small><h2>Comparador de política</h2></div><span class="status-pill">SOMENTE LOCAL</span></div>
    <div class="app-content"><p>Registre primeiro a linha de base e depois envie uma variante estruturada. Nenhum payload é executado: o motor apenas compara estados declarativos.</p>
      <button id="academy-baseline" class="ghost" type="button">Registrar linha de base</button>
      <form id="academy-form"><label>Variante<input id="academy-variant" name="variant" autocomplete="off" placeholder="variant-..."></label><label>Evidence ID<input id="academy-evidence" name="evidenceId" autocomplete="off" placeholder="...-evidence-..."></label><button class="primary" type="submit">Comparar variante</button></form>
    </div></section>`;
}

function applicationFor(lab) {
  if (lab.payload?.startsWith('/academy/')) return academyApplication(lab);
